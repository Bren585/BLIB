...
namespace BLIB {
	...
	namespace base {
		...
		public:
			...
			/* Globals */
			static void SetGlobalGravity			(V g) { gravity = g; }
			static void SetGlobalTerminalVelocity	(V t) { tvel	= t; }

			static V GetGlobalGravity() { return gravity; }
...
#define FULL_BASE base::entity<float3, float3>
		...
		public:
			...
			float3 get_ang() const { return object::get_ang(); }
			...
			float3& imgui_get_ang() { return object::imgui_get_ang(); }
			...
			void mlt_ang(float3 d) { object::mlt_ang(d); }
