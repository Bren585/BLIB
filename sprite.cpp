...
void BLIB::update_point_buffer(ID3D11Buffer* point_buffer, float2 pos, float2 scale, float2 pivot, float rotation, float2 tile_index) {
	HRESULT hr{ S_OK };
...
void BLIB::make_constant_buffer(ID3D11Buffer** out) {
	D3D11_BUFFER_DESC buffer_desc{};
	buffer_desc.ByteWidth			= sizeof(sprite::constants);
	...
	HRESULT hr = device::get()->CreateBuffer(&buffer_desc, nullptr, out); VERIFY;
}

void BLIB::update_constant_buffer(ID3D11Buffer** constant_buffer_addr, float2 texture_size, color c, float2 tile_size) {
	sprite::constants data;
	data.color			= c;
	data.tile_size		= tile_size;
	data.texture_size	= texture_size;
	data.viewport		= sprite::get_viewport();
	data.y_invert		= sprite::get_y_invert();
	data.dummy			= 0;
	device::context()->UpdateSubresource(*constant_buffer_addr, 0, 0, &data, 0, 0);
	device::context()->PSSetConstantBuffers(FLAT_COMMON_CB, 1, constant_buffer_addr);
	device::context()->GSSetConstantBuffers(FLAT_COMMON_CB, 1, constant_buffer_addr);
}
...
void sprite::update_vertex_buffer(float2 pos, float2 scale, float2 pivot, float rotation, float2 tile_index) {
	update_point_buffer(vertex_buffer.Get(), pos, scale, pivot, rotation, tile_index);
}

void sprite::update_constant_buffer(color color, float2 tile_size) {
	::update_constant_buffer(constant_buffer.GetAddressOf(), get_size(), color, tile_size);
}
...
sprite::sprite(flags flags, const string& filename) {
	...
	if (flags & load_shaders	) { load_shader( flags & variable ? VARIABLE_VS : SPRITE_VS);	}
	if (flags & variable		) { shader::load_gs(VARIABLE_GS);								}				
	if (flags & load_texture	) { sprite::load_file(filename, (flags & full_filename));		} else { resize(float2{1}); } // Make sure size is never 0
}
...
void sprite::render(
	...
	update_vertex_buffer(pos, scale, pivot, rotation, tile_index);
